// Simple simulation of the tool-router fallback invoking a read_any_file tool
async function main() {
  const state: any = {
    chatId: "chat-sim",
  ...
  function hasAnyActiveAttachment(state: any): boolean {
    return (state.attachments ?? []).some((attachment: any) => attachment.status !== "failed");
  }

  function shouldPreferAttachmentExtractionIntent(state: any): boolean {
  ...
  if (shouldPreferAttachmentExtractionIntent(state)) {
    const firstAttachmentId = (state.messageAttachmentIds ?? [])[0] ?? (state.attachments ?? []).find((a: any) => a.status !== "failed")?.id;
    console.log("Heuristic triggered; would invoke read_any_file for:", firstAttachmentId);

    // Mock read_any_file tool
    const readTool = {
      invoke: async ({ attachmentId }: { attachmentId: string }) => {
