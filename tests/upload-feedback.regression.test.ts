...
test("upload handlers do not show success toast for failed attachment status", () => {
  ...
  assert.match(homeSource, /Upload completed, extraction failed/);
  assert.match(chatSource, /Upload completed, extraction failed/);
  ...
  assert.match(attachmentProcessingSource, /Uploaded, but extraction failed:/);
  assert.doesNotMatch(
    attachmentProcessingSource,
    /if \(!shouldDeferParsing\)[\s\S]*status = "failed"/,
  );
});